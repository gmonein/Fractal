...
/*   Updated: 2017/03/23 16:54:25 by gmonein          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ftui_switch_p_out(t_ui_elem *s, t_ui_image *ui, t_p *p)
{
	mlx_put_image_to_window(p->mlx, p->win, \
			s->img[0 + (*(int *)s->elema != 0 ? 1 : 0)], s->pos_x, s->pos_y);
}

void			ftui_switch_p_in(t_ui_elem *s, t_ui_image *ui, t_p *p)
{
	mlx_put_image_to_window(p->mlx, p->win, \
			s->img[2 + (*(int *)s->elema != 0 ? 1 : 0)], s->pos_x, s->pos_y);
}
...
t_ui_elem		*ftui_create_switch(t_ui_elem inf, char *path, t_p *p)
{
	...
	s->fct[4] = NULL;
	*(int *)s->elema = \
		(*(int *)s->elema != 0 && *(int *)s->elema != 1 ? 0 : *(int *)s->elema);
	return (s);
}
