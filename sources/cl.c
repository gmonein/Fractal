...
void		init_clmem(t_cl *cl, t_all *a)
{
	int		i;
...
void				cl_init(t_all *env)
{
	t_kernel_init		init;

	init = (t_kernel_init){KRN_NAME,
									KRN_NBSOURCE,
									KRN_SRCPATH,
									KRN_NBARG,
									KRN_MEMARG,
	...
	init.name = "julia";
	cl_create_kernel(&env->cl, &env->cl.kernels[KRN_JULIA_ID], &init);
	init.name = "mandelbrot";
	cl_create_kernel(&env->cl, &env->cl.kernels[KRN_MDLB_ID], &init);
	init.name = "island";
